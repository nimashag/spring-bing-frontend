...
const NewArrivalList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/product")
      .then((res) => res.json())
      .then((data) => {
        // Sort products by their creation date (or other timestamp) in descending order
        const sortedProducts = data.data.sort(
          (a: Product, b: Product) =>
            new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );

        // Get the last 12 products for the New Arrivals section
        const last12Products = sortedProducts.slice(0, 12);
        setProducts(last12Products);
      })
      .catch((error) => {
