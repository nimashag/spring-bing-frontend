...
const SalesForecasting: React.FC = () => {
  ...
  useEffect(() => {
    const fetchForecastData = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/sales-forecasting');
  ...
  const actualSales = forecastData.map((item) => item.total_sales_current_month);
  ...
  return (
    <div className="container mx-auto p-4">
        ...
        <tbody>
          {forecastData.map((item) => (
            <tr key={item.product_id} className="hover:bg-gray-100">
              ...
              <td className="py-2 px-4 border">{item.predicted_sales.toFixed(2)}</td>
              <td className="py-2 px-4 border">{item.total_sales_current_month}</td>
              <td className="py-2 px-4 border">{item.growth_rate ? item.growth_rate.toFixed(2) : '0'}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="mb-8">
        <h3 className="text-xl font-bold mb-4">Predicted vs Actual Sales</h3>
        <Bar
          data={{
            labels: productNames,
            datasets: [
      ...
      <div className="mb-8">
        <h3 className="text-xl font-bold mb-4">Sales Distribution by Product</h3>
        <Pie
          data={{
            labels: productNames,
            datasets: [
              {
                label: 'Total Sales',
                data: totalSales.map(sales => (sales / totalSalesSum) * 100),
                backgroundColor: [
                  'rgba(255, 99, 132, 0.6)',
                  'rgba(54, 162, 235, 0.6)',
                  'rgba(255, 206, 86, 0.6)',
          ...
          options={{
            ...
            plugins: {
              tooltip: {
                callbacks: {
                  label: (tooltipItem: any) => `${tooltipItem.label}: ${tooltipItem.raw.toFixed(2)}%`,
                },
              },
            },
          }}
      ...
      <div className="mb-8">
        <h3 className="text-xl font-bold mb-4">Sales Comparison (Predicted vs Actual vs Growth Rate)</h3>
        <Radar
          data={{
            labels: productNames,
            datasets: [
