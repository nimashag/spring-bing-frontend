...
const PriceSuggestions: React.FC = () => {
    ...
    const fetchPriceSuggestions = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3001/api/price-suggestions"
        );
  ...
  const handleAcceptNewPrice = async () => {
    if (selectedProduct && newPrice) {
      try {
        await axios.put(
          `http://localhost:3001/api/${selectedProduct.product_id}`,
          {
            newPrice,
          }
        );
        ...
        <div ref={printRef}>
          <div className="container mx-auto p-4">
              ...
              <tbody>
                {currentSuggestions.map((suggestion) => (
                  <tr key={suggestion.product_id} className="hover:bg-gray-100">
                    ...
                    <td className="py-2 px-4 border">
                      {suggestion.stock_status}
                    </td>
                    <td className="py-2 px-4 border">
                      {new Date(suggestion.date_suggested).toLocaleDateString()}
                    </td>
                    <td className="py-2 px-4 border">
                      <button
            ...
            <div className="flex justify-center mb-4">
              {Array.from(
                { length: Math.ceil(suggestions.length / suggestionsPerPage) },
                (_, index) => (
                  <button
                    key={index}
              ...
              <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                <div className="bg-white p-5 rounded shadow-lg">
                  <h3 className="text-xl font-bold mb-4">
                    {selectedProduct.product_name} Details
                  </h3>
                  <div>
                    <p>
                      Current Price: {selectedProduct.current_price.toFixed(2)}
                    </p>
                    <p>
                      Suggested Price:{" "}
                      {selectedProduct.suggested_price.toFixed(2)}
                    </p>
                    <p>Stock Status: {selectedProduct.stock_status}</p>
                    <p>
                      Date Suggested:{" "}
                      {new Date(
                        selectedProduct.date_suggested
                      ).toLocaleDateString()}
                    </p>
                    <input
                      type="number"
